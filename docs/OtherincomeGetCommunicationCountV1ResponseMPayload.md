# OtherincomeGetCommunicationCountV1ResponseMPayload

Response for GET /1/object/otherincome/{pkiOtherincomeID}/getCommunicationCount

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**iCommunicationCount** | **number** | The count of Communication. | [default to undefined]

## Example

```typescript
import { OtherincomeGetCommunicationCountV1ResponseMPayload } from '@ezmaxinc/ezmax-sdk-typescript-axios';

const instance: OtherincomeGetCommunicationCountV1ResponseMPayload = {
    iCommunicationCount,
};
```

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)
